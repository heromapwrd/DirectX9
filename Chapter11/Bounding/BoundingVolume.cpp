...
ID3DXMesh* pSephere = NULL;
ID3DXMesh* pBox = NULL;
bool RenderBoundingSephere = true;
...
bool ComputeBoundingSehpere(ID3DXMesh* pMesh, BoundingSphere* pSephere);
bool ComputeBoundingBox(ID3DXMesh* pMesh, BoundingBox* pBox);
...
void CleanUp()
{
	...
	d3d9::Release<ID3DXMesh*>(pSephere);
	d3d9::Release<ID3DXMesh*>(pBox);
}
...
bool SetUp()
{
	...
	BoundingSphere sephere;
	BoundingBox box;

	if (ComputeBoundingSehpere(pMesh, &sephere))
	{
		hr = D3DXCreateSphere(
			g_pDevice,
			sephere._radius,
		...
		if (FAILED(hr))
		{
			MessageBox(0, "Failed to create sephere!", 0, 0);
			return false;
		}

		OutFile.open("Sepher_Dump.txt");

		dumpVertices(OutFile, pSephere);
		dumpIndex(OutFile, pSephere);
		dumpAttributeBuffer(OutFile, pSephere);
		dumpAdjacencyBuffer(OutFile, pSephere);
		dumpAttributeTable(OutFile, pSephere);

		OutFile.close();
	}

	if (ComputeBoundingBox(pMesh,&box))
	{
		hr = D3DXCreateBox(
			g_pDevice,
			box._max.x - box._min.x,
			box._max.y - box._min.y,
			box._max.z - box._min.z,
			&pBox,
			NULL);
		if (FAILED(hr))
		{
			MessageBox(0, "Failed to create box!", 0, 0);
			return false;
		}

		OutFile.open("Box_Dump.txt");

		dumpVertices(OutFile, pBox);
		dumpIndex(OutFile, pBox);
		dumpAttributeBuffer(OutFile, pBox);
		dumpAdjacencyBuffer(OutFile, pBox);
		dumpAttributeTable(OutFile, pBox);
...
bool DisPlay(DWORD timeDelta)
{
	...
	if (g_pDevice)
	{
		...
		D3DMATERIAL9 Bule = d3d9::BLUE_MTRL;
		Bule.Diffuse.a = 0.1f;
		g_pDevice->SetMaterial(&Bule);
		...
		if (RenderBoundingSephere)
		{
			if (pSephere)
			{
				pSephere->DrawSubset(0);
			}
		}
		else
		{
	...
	LPARAM lParam)
{
	...
	case WM_KEYDOWN:
		if (wParam == VK_ESCAPE)
			DestroyWindow(hWnd);
		if (wParam == VK_SPACE)
			RenderBoundingSephere = !RenderBoundingSephere;
		break;
	}
...
void dumpVertices(ofstream& OutFile, ID3DXMesh* pMesh)
{
	...
	DWORD size = pMesh->GetNumVertices();
