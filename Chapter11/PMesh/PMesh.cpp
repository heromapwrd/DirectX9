...
void CleanUp()
{
	...
	d3d9::Release<ID3DXPMesh*>(pPMesh);
}
...
bool SetUp()
{
	...
	d3d9::Release<ID3DXBuffer*>(pAdjBuffer);

	if (FAILED(hr))
	{
		MessageBox(0, "Generate Progressive Mesh Failed!", 0, 0);
		return false;
	}
	...
	// 初始化视觉矩阵,投影矩阵
	// 相机位置,视点位置,竖直向上向量
	D3DXVECTOR3 eye(-15.0f, 3.0f, -15.0f);
...
bool DisPlay(DWORD timeDelta)
{
	...
	if (g_pDevice)
	{
		...
		DWORD numFaces = pPMesh->GetNumFaces();

		if (GetAsyncKeyState('A') & 0x8000f)
		{
			pPMesh->SetNumFaces(numFaces + 1);
			if (numFaces == pPMesh->GetNumFaces())
			{
		...
		for (int i = 0; i < Textures.size(); i++)
		{
			...
			g_pDevice->SetMaterial(&d3d9::YELLOW_MTRL);
			pPMesh->DrawSubset(i);
